/**
 * This program demonstrates heap sort algorithm
 * implementation in c++.
*/
...
std::vector<int> heap_sort(std::vector<int> my_heap)
{
    int index = 0;
    ...
    while(my_heap.size()>0)
    {
        ...
        sorted_list.insert(sorted_list.begin(), my_heap[0]);
        // delete the root node after extracting and heapify again
        delete_node(my_heap[0], my_heap);
    }
...
int main()
{
    ...
    sorted_list = heap_sort(my_heap);
    print_heap(sorted_list);
